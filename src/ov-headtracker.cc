...
int main(int argc, char** argv)
{
  ...
  std::string waccpath("/wacc");
  std::string calib0path("/scene/ego/0/ap1/sndfile/loop");
  std::string calib1path("/scene/ego/0/ap0/sndfile/loop");
  //
  const char* options = "p:r:1:0:s:hc:2:x:y:z:";
  struct option long_options[] = {
      {"port", 1, 0, 'p'},    {"rpath", 1, 0, 'r'}, {"c1path", 1, 0, '1'},
      {"c0path", 1, 0, '0'},  {"xaxis", 1, 0, 'x'}, {"yaxis", 1, 0, 'y'},
      {"zaxis", 1, 0, 'z'},   {"scale", 1, 0, 's'}, {"help", 0, 0, 'h'},
      {"accpath", 1, 0, 'c'}, {"port2", 1, 0, '2'}, {0, 0, 0, 0}};
  int opt(0);
  int option_index(0);
  ...
  lo_address loa2(NULL);
  if(!port2.empty()) {
    loa2 = lo_address_new("localhost", port2.c_str());
    if(!loa2)
      throw ErrMsg("Invalid secondary lo address");
  }
  ...
  while(true) {
    try {
      ...
      ax_x = std::min(ax_x, (unsigned int)(data.size() - 1));
      ax_y = std::min(ax_y, (unsigned int)(data.size() - 1));
      ax_z = std::min(ax_z, (unsigned int)(data.size() - 1));
      while(true) {
        ...
        if(l.size()) {
          ...
          case 'A': {
            ...
            data[2] = std::stod(l);
            data[3] = 0.0;
            lo_send(loa, accpath.c_str(), "fff", data[ax_x], data[ax_y],
                    data[ax_z]);
            if(loa2)
              lo_send(loa2, accpath.c_str(), "fff", data[ax_x], data[ax_y],
                      data[ax_z]);
          } break;
          case 'W': {
            l = l.substr(1);
            ...
            data[2] = std::stod(l);
            data[3] = 0.0;
            lo_send(loa, waccpath.c_str(), "fff", data[ax_x], data[ax_y],
                    data[ax_z]);
            if(loa2)
              lo_send(loa2, waccpath.c_str(), "fff", data[ax_x], data[ax_y],
                      data[ax_z]);
          } break;
          case 'R': {
            l = l.substr(1);
            ...
            l = l.substr(sz + 1);
            data2[6] = std::stod(l);
            lo_send(loa, "/raw", "fffffff", data2[0], data2[1], data2[2],
                    data2[3], data2[4], data2[5], data2[6]);
            if(loa2)
              lo_send(loa2, "/raw", "fffffff", data2[0], data2[1], data2[2],
                      data2[3], data2[4], data2[5], data2[6]);
          } break;
          case 'G': {
            l = l.substr(1);
            ...
            data[2] = std::stod(l);
            data[3] = 0.0;
            lo_send(loa, path.c_str(), "fff", (float)(scale * data[ax_z]),
                    (float)(scale * data[ax_y]), (float)(scale * data[ax_x]));
            if(loa2)
              lo_send(loa2, path.c_str(), "fff", (float)(scale * data[ax_z]),
                      (float)(scale * data[ax_y]), (float)(scale * data[ax_x]));
          } break;
          case 'C': {
            if(l.size() > 1) {
              if((l[1] == '1') && calib1path.size()) {
                ...
                if(loa2)
                  lo_send(loa2, calib1path.c_str(), "i", 1);
              }
              if((l[1] == '0') && calib0path.size()) {
                lo_send(loa, calib0path.c_str(), "i", 1);
                if(loa2)
                  lo_send(loa2, calib0path.c_str(), "i", 1);
              }
            }
          } break;
          default: {
